const Review = (() => {
  ...
  function answer(rating){
    ...
    if(live){
      const level={hard:'recognize',medium:'assisted',easy:masteryOf(live)==='active'?'active':'ready'}[rating];
      if(masteryOf(live)!==level){
        DB.learningEvents ||= [];
        DB.learningEvents.push({at:new Date().toISOString(),id:live.id,type:item.kind==='words'?'word':'phrase',from:masteryOf(live),to:level});
      }
      live.mastery=level;live.status=MASTERY[level].status;
      live.reviewCount=(live.reviewCount||0)+1;live.nextReview=new Date(Date.now()+({hard:1,medium:3,easy:7}[rating])*86400000).toISOString();live.lastReviewedAt=new Date().toISOString();saveDB();updateNavBadges();showToast(`Estado actualizado: ${MASTERY[level].label}`);}
